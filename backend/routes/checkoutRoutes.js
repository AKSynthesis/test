...
// @desc Update checkout to mark as pay after succesful payment
// @access Private
...
router.post("/:id/finalize", protect, async (req, res) => {
    try {
        ...
        if (checkout.isPaid === true && !checkout.isFinalized) {
            ...
            // Mark chekout session as finalized 
            ...
            res.status(400).json({ message: "Checkout session not paid" });
        }
    } catch (error) {
        console.error(error);
        res.status(500).send("Server Error");

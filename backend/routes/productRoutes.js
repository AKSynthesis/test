...
router.get("/", async (req, res) => {
    try {
        ...
        // Fetch products from BD, apple sorting and limit result
...
router.get("/best-seller", async (req, res) => {
    try {
        const bestSellerProduct = await Product.findOne()
            .sort({ rating: -1 }); // Sort by rating in descending order (highest first)
...
router.get("/new-arrivals", async (req, res) => {
    try {
        ...
        if (newArrivals) {
            ...
            res.status(404).json({ message: "No new arrival" })
        }
    } catch (error) {
        console.error(error);
        res.status(500).send("Server error");
...
// @desc GET a single product by ID from DB
// @access Public
